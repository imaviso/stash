...
class S3Service {
    ...
    suspend fun initialize(config: S3Config) = withContext(Dispatchers.IO) {
    ...
    private fun requireClient(): S3Client {
        return client ?: throw IllegalStateException("S3 client not initialized. Please configure credentials first.")
    }
    
    // ==================== BUCKET OPERATIONS ====================
    
    suspend fun listBuckets(): Result<List<S3Bucket>> = withContext(Dispatchers.IO) {
        runCatching {
    ...
    suspend fun createBucket(name: String): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
            Log.d(TAG, "Creating bucket: $name")
            requireClient().createBucket(CreateBucketRequest {
                bucket = name
            })
    ...
    suspend fun deleteBucket(name: String): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
            Log.d(TAG, "Deleting bucket: $name")
            requireClient().deleteBucket(DeleteBucketRequest {
                bucket = name
            })
    ...
    suspend fun listObjects(
        ...
        runCatching {
            ...
            val response = requireClient().listObjectsV2(ListObjectsV2Request {
                bucket = bucketName
                this.prefix = prefix
    ...
    suspend fun uploadObject(
        ...
        runCatching {
            ...
            requireClient().putObject(PutObjectRequest {
                bucket = bucketName
                this.key = key
    ...
    suspend fun downloadObject(
        ...
        runCatching {
            ...
            val response = requireClient().getObject(GetObjectRequest {
                bucket = bucketName
                this.key = key
            }) { response ->
    ...
    suspend fun downloadObjectToFile(
        ...
        runCatching {
            ...
            val presignedUrl = if (config.usePathStyle) {
    ...
    suspend fun renameObject(
        ...
        runCatching {
            ...
            // Copy to new key
            requireClient().copyObject(CopyObjectRequest {
            ...
            // Delete old key
            requireClient().deleteObject(DeleteObjectRequest {
                bucket = bucketName
                key = oldKey
            })
    ...
    suspend fun deleteObjects(
        ...
        runCatching {
            ...
            requireClient().deleteObjects(DeleteObjectsRequest {
                bucket = bucketName
                delete = Delete {
                    objects = keys.map { keyName ->
    ...
    private fun generateManualPresignedUrl(
        ...
        val dateTimeFormat = java.time.format.DateTimeFormatter
        ...
        val host = if (endpointUrl.port != -1 && endpointUrl.port != 80 && endpointUrl.port != 443) {
